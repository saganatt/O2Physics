...
/// \file simpleApplyPidSophieModel
...
#include "Tools/PIDML/pidSophieModel.h"
...
struct SimpleApplySophieModel {
  PidSophieModel pidModel; // One instance per model, e.g., one per each pid to predict
  ...
  void init(InitContext const&)
  {
    if (cfgUseCCDB) {
      ...
      pidModel = PidSophieModel(cfgPathLocal.value, cfgPathCCDB.value, cfgUseCCDB.value, ccdbApi, -1, cfgPid.value, static_cast<PidMLDetector>(cfgDetector.value), cfgCertainty.value);
    }
  }
  ...
  void processCollisions(aod::Collisions const& collisions, BigTracks const& tracks, aod::BCsWithTimestamps const&)
  {
    ...
    if (cfgUseCCDB && bc.runNumber() != currentRunNumber) {
      ...
      pidModel = PidSophieModel(cfgPathLocal.value, cfgPathCCDB.value, cfgUseCCDB.value, ccdbApi, timestamp, cfgPid.value, static_cast<PidMLDetector>(cfgDetector.value), cfgCertainty.value);
    }
  ...
  PROCESS_SWITCH(SimpleApplySophieModel, processCollisions, "Process with collisions and bcs for CCDB", true);
  ...
  PROCESS_SWITCH(SimpleApplySophieModel, processTracksOnly, "Process with tracks only -- faster but no CCDB", false);
};

WorkflowSpec defineDataProcessing(ConfigContext const& cfgc)
{
  return WorkflowSpec{
    adaptAnalysisTask<SimpleApplySophieModel>(cfgc)};
}
