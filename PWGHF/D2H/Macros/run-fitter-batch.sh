...
for dir in ${INPUT_PATTERN}${PERM_PATTERN}* ; do
  ...
  echo "fd ${probs[3]} ${probs[5]} ${probs[6]} ${probs[7]} ${probs[8]} ${probs[9]} ${probs[10]} ${probs[11]}"
  ...
  sed -i "s/%sf23%/${probs[3]}/g" "${CUR_CFG}" || exit 1
  sed -i "s/%sf34%/${probs[3]}/g" "${CUR_CFG}" || exit 1
  sed -i "s/%sf45%/${probs[3]}/g" "${CUR_CFG}" || exit 1
  sed -i "s/%sf56%/${probs[3]}/g" "${CUR_CFG}" || exit 1
